...
generate_chart_file() {
    local chart_template="${SCRIPT_DIR}/templates/Chart.yaml.template"
    local chart_file="${SCRIPT_DIR}/Chart.yaml"
    
    print_info "Generando Chart.yaml con versiones de controladores..."
    
    if [[ ! -f "$chart_template" ]]; then
        print_error "Template de Chart.yaml no encontrado: $chart_template"
        exit 1
    fi
    
    # Generar Chart.yaml con las versiones del .env
    envsubst < "$chart_template" > "$chart_file"
    
    print_success "Chart.yaml generado con versiones: LBC=${LB_CONTROLLER_VERSION}, CA=${CLUSTER_AUTOSCALER_VERSION}"
}
